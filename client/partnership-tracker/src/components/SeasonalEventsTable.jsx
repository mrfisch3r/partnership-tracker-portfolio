...
const SeasonalEventsTable = ({
    ...
    const fetchEvents = async () => {
      try {
        ...
        const res = await fetch(
          "http://localhost:5001/api/get_seasonal_events",
          {
            headers: {
  ...
  const filtered = events.filter((event) => {
    // Safety check for filters object - must be first!
  ...
  return (
    <div className="partnership-table">
      ...
      {sorted.length === 0 ? (
        <p>
          No seasonal events found. Add your first event using the "Add New
          Event" button.
        </p>
      ) : (
