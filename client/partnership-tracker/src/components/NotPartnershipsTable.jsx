...
const NotPartnershipsTable = ({ filters, onPartnerSelect, refreshTrigger = 0 }) => {
  ...
  useEffect(() => {
    async function fetchPartners() {
      try {
        ...
        setError('Failed to load not potential partnerships. Please try again later.');
        setLoading(false);
      }
    }
    fetchPartners();
  ...
  return (
    <div className="partnership-table">
      ...
      {sorted.length === 0 ? (
        <p>No "not potential partnerships" found. Add your first entry using the "Add New Not Potential Partner" button.</p>
      ) : (
        <table>
          <thead>
            <tr>
            ...
            {sorted.map((partner) => {
              ...
              return (
                ...
                >
                  ...
                  <td>{truncateText(partner.contact_attempt, 30)}</td>
                  <td>
