...
const MonthlyUpdatesTable = ({ filters, onUpdateSelect, refreshTrigger = 0 }) => {
  ...
  return (
    <div className="partnership-table">
      ...
      {sorted.length === 0 ? (
        <p>No monthly updates found. Add your first update using the "Add New Monthly Update" button.</p>
      ) : (
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            {sorted.map((update) => (
              ...
              >
                ...
                <td>{truncateText(update.major_findings)}</td>
                <td>{truncateText(update.barriers_and_solutions)}</td>
                <td>
